...
async def test_read_read_scenario_reports_snapshots(orchestrator):
    """READ_READ scenario: Two readers should both get snapshots and delta = 0."""
...
async def test_read_write_applies_new_quantity(orchestrator):
    """READ_WRITE scenario: Writer should update the quantity."""
    coordinator, _ = orchestrator
    payload = OrchestrationInput(
...
@pytest.mark.asyncio
async def test_write_write_last_writer_wins(orchestrator):
    """WRITE_WRITE scenario: Both writers commit, last one sets final quantity."""
    coordinator, _ = orchestrator
    payload = OrchestrationInput(
    ...
    assert state.status == "completed"
    assert state.client_status["writer_a"]["status"] == "committed"
    assert state.client_status["writer_b"]["status"] == "committed"
    # Final quantity should be one of the two written values
    assert state.summary["final_quantity"] in (10, 20)


@pytest.mark.asyncio
async def test_write_write_serialization_conflict(orchestrator):
    """WRITE_WRITE with serialization failure should track conflict."""
...
@pytest.mark.asyncio
async def test_isolation_level_overview_in_summary(orchestrator):
    """Summary should include isolation_overview with each actor's level."""
    coordinator, _ = orchestrator
    payload = OrchestrationInput(
        scenario=ScenarioType.READ_READ,
        actors=[
    ...
    overview = state.summary["isolation_overview"]
    assert overview["r1"] == "READ COMMITTED"
    assert overview["r2"] == "REPEATABLE READ"


@pytest.mark.asyncio
async def test_abort_run_sets_status(orchestrator):
    """Aborting a run should set status to aborted."""
    coordinator, _ = orchestrator
    payload = OrchestrationInput(
        scenario=ScenarioType.READ_READ,
        actors=[
            TransactionActorInput(name="r1", node="node0", isolation_level=IsolationLevel.READ_COMMITTED),
            TransactionActorInput(name="r2", node="node0", isolation_level=IsolationLevel.READ_COMMITTED),
        ],
    )
    state = await coordinator.start_run(payload)
    # Immediately abort
    await coordinator.abort_run(state.id)

    status = coordinator.get_status(state.id)
