"""Transaction orchestrator for demonstrating distributed concurrency scenarios.

This module implements TRUE concurrent PostgreSQL transactions to demonstrate
isolation-level visibility rules. Each transaction uses BEGIN ... COMMIT with
pg_sleep() executed INSIDE the transaction to create overlap windows.

Scenarios
---------
* READ_READ  – Two readers observe the same row; demonstrates visibility.
* READ_WRITE – A reader races a writer; demonstrates non-repeatable reads.
* WRITE_WRITE – Two writers lock the same row; demonstrates serialization.

Key behaviors
-------------
* pg_sleep runs INSIDE the transaction (between BEGIN and COMMIT).
* Each actor has its own isolation level.
* Proper PostgreSQL locking is observed (FOR SHARE, FOR UPDATE).
"""
...
class TransactionOrchestrator:
    """Coordinates TRUE concurrent SQL transactions across selectable nodes.
    
    Each actor runs in its own asyncpg connection with:
    - BEGIN TRANSACTION ISOLATION LEVEL <level>
    - pg_sleep(<delay>) executed INSIDE the transaction
    - Proper locking (FOR SHARE for reads, FOR UPDATE for writes)
    - COMMIT/ROLLBACK
    
    All actors run in parallel via asyncio.gather to create real concurrency.
    """
    ...
    async def _execute_run(self, state: RunState) -> None:
        try:
            ...
            # Launch all actors in parallel for TRUE concurrency
    ...
    async def _execute_actor(self, state: RunState, plan: ActorPlan, order_id: UUID) -> None:
        """Execute a single actor's transaction with TRUE pg_sleep inside the transaction."""
        ...
        conn = await self._connection_factory(dsn)
        try:
            # START TRANSACTION with the requested isolation level
            await conn.execute(
            ...
            # Execute the appropriate operation based on role
            if plan.role == "read":
            ...
            await state.log("transaction_cancelled", actor_id=plan.actor_id)
            raise
    ...
    async def _perform_read(
        ...
        """Execute a read operation with FOR SHARE lock, then pg_sleep, then re-read."""
        # First read with FOR SHARE to acquire shared lock
        ...
        await state.log(
            ...
            actor_id=plan.actor_id,
            quantity=qty_before,
            description="acquired FOR SHARE lock",
        )
        # pg_sleep INSIDE the transaction - this is the key difference
        if plan.delay_seconds > 0:
            ...
            await state.log(
                ...
                description="pg_sleep executed inside transaction",
            )
        # Second read to see visibility effects based on isolation level
    ...
    async def _perform_write(
        ...
        """Execute a write operation with FOR UPDATE lock, then pg_sleep, then UPDATE."""
        # Acquire exclusive lock with FOR UPDATE
        row = await conn.fetchrow(
        ...
        await state.log(
            ...
            description="acquired FOR UPDATE lock",
        )
        # pg_sleep INSIDE the transaction while holding the lock
        if plan.delay_seconds > 0:
            await conn.execute("SELECT pg_sleep($1)", plan.delay_seconds)
            await state.log(
                "pg_sleep_completed",
                actor_id=plan.actor_id,
                seconds=plan.delay_seconds,
                description="pg_sleep executed inside transaction with lock held",
            )
        # Perform the UPDATE
        await conn.execute(
